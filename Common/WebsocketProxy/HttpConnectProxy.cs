...
namespace Common.WebsocketProxy
{
    /// <remarks>Taken from https://github.com/kerryjiang/SuperSocket.ClientEngine/blob/master/Proxy/HttpConnectProxy.cs </remarks>
...
//        private const string m_RequestTemplate = "CONNECT {0}:{1} HTTP/1.1\r\nHost: {0}:{1}\r\nProxy-Connection: Keep-Alive\r\n\r\n";
        private readonly string m_RequestTemplate;

//        private const string m_ResponsePrefix = "HTTP/1.1";
        private readonly string m_ResponsePrefix;
...
#else
        /// <summary>Create new proxy object for websocket communication</summary>
        /// <param name="proxyEndPoint">Proxy end-point</param>
        /// <param name="httpVersion">HTTP version both for request and response. Must be one of "1.0" or "1.1"</param>
        public HttpConnectProxy(EndPoint proxyEndPoint, string httpVersion = "1.1")
            : this(proxyEndPoint, 128)
        {
            if ("1.0" != httpVersion && "1.1" != httpVersion)
            {
                throw new ArgumentException("Invalid HTTP version. Expected one of [\"1.0\", \"1.1\"]", "httpVersion");
            }

            m_RequestTemplate = "CONNECT {0}:{1} HTTP/" + httpVersion + "\r\nHost: {0}:{1}\r\nProxy-Connection: Keep-Alive\r\n\r\n";
            m_ResponsePrefix = "HTTP/" + httpVersion;
        }
